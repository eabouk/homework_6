...
## We've provided the function access_third_elem. Write a lambda function that does exactly the same thing as access_third_elem 
##(you can assume any input it would receive has at least 3 elements in it). Assign that lambda function to the variable sample_func.

## Note that we cannot specifically test in the unit tests whether it is a lambda function, but you will not get points for this 
...
# Then write a line of code to cast the iterator to a list (it should end up as a list of tuples). Save that list in a variable 
## called student_tups_list.
student_tups_list = list(student_tups)

## You can test this out with any code you like here, and similar below other problems, but make sure to comment out any code that 
## uses up the iterator in order to pass the tests!
#print(student_tups_list)    


## [PROBLEM 5]
print("\n\n***** Problem 5 *****")
# Use a list comprehension to create a list of Student instances out of the student_tups list you just created in Problem 2, 
...
## Define a function called readfiles, which accepts a list of filenames as input and yields each line in each of the file with that 
...
## There is a test for this but an even more fun test is to uncomment the code below which invokes the main_filterer function and 
## prints each line from the generator without blank lines in between (that's what the comma is doing).
...
## so you can test with those file names! The test below also relies upon these files. Of course, you could also create other 
## files for testing.
